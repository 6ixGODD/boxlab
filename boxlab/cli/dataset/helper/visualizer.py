...
class DatasetVisualizer:
    ...
    def _extract_source_from_filename(self, filename: str) -> str | None:
        """Extract source prefix from filename (before first
        underscore).
    ...
    def visualize_split_comparison(self) -> pathlib.Path:
        ...
        ax3.set_title("Avg Annotations per Image", fontsize=14, fontweight="bold")
        ...
        ax4.set_title("Category Distribution Comparison", fontsize=14, fontweight="bold")
        ax4.set_xticks(x + width * (len(self.datasets) - 1) / 2)
        ax4.set_xticklabels(all_categories_list, rotation=45, ha="right")
        ax4.legend()
    ...
    def visualize_source_distribution(self) -> pathlib.Path | None:
        """Visualize distribution by source extracted from filename
        prefixes.
        ...
        # Images by source
        bars1 = ax1.bar(sources, image_counts, color=colors, edgecolor="black", alpha=0.8)
        for bar in bars1:
            height = bar.get_height()
            ax1.text(
                bar.get_x() + bar.get_width() / 2,
        ...
        ax1.set_title("Images by Source (Filename Prefix)", fontsize=14, fontweight="bold")
        ...
        bars2 = ax2.bar(sources, ann_counts, color=colors, edgecolor="black", alpha=0.8)
        for bar in bars2:
            height = bar.get_height()
            ax2.text(
                bar.get_x() + bar.get_width() / 2,
        ...
        ax2.set_title("Annotations by Source (Filename Prefix)", fontsize=14, fontweight="bold")
    ...
    def visualize_annotation_heatmap(self) -> pathlib.Path | None:
        """Generate annotation density heatmap with blue-yellow
        colormap.
        ...
        im = ax.imshow(heatmap, cmap="YlOrBr", interpolation="bilinear", origin="upper")
        ax.set_title(
